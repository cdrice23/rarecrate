...
const initCronRun = async () => {
  ...
  // Create an array of all crates created by a profile with followers and create a notification for each follower
  ...
  for (let followerData of allFollowerData) {
    ...
    for (let notification of notifications) {
      for (let follower of followerData.followers) {
        await prisma.notification.create({
          data: {
            receiver: follower,
            type: 'newFavorite',
            actionOwner: followerData.profileId,
            connectedCrate: {
              connect: {
                id: notification.crateId,
              },
            },
          },
        });
